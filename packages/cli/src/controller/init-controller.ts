...
import {ProjectManifestV0_0_1, SubstrateProjectManifestV0_2_0} from '@subql/common-substrate';
...
export async function readDefaults(projectPath: string): Promise<string[]> {
  ...
  const currentProject = yaml.load(manifest) as ProjectManifestV0_0_1 | SubstrateProjectManifestV0_2_0;
  return [
...
async function prepareManifest(projectPath: string, project: ProjectSpecBase): Promise<void> {
  ...
  const yamlPath = path.join(`${projectPath}`, `project.yaml`);
  const manifest = await fs.promises.readFile(yamlPath, 'utf8');
  const data = yaml.load(manifest) as ProjectManifestV0_0_1 | SubstrateProjectManifestV0_2_0;
