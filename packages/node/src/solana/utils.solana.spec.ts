...
describe('SolanaUtils', () => {
  describe('IDLs', () => {
    ...
    it('caches IDLs', async () => {
      ...
      await decodeLog(solanaApi, '', undefined, 'JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4');
    ...
    describe('decode instruction', () => {
      ...
      it('can decode an instruction with an IDL file', async () => {
        ...
        const tx = blockData.transactions.find(tx => tx.transaction.signatures.find(s => s === '3rf2sSMeJC1dd4t4TDvYPfvQjpL6DG6qdDcMnruDtATPbwjqt3xDnNvddtiTBESL8AWiFt2zENghqAh1h252bKQi'))
        const instruction = tx?.transaction.message.instructions[3];

        const decoded = await decodeInstruction(solanaApi, instruction as any, IDL_Jupiter)
    ...
    describe('decode log', () => {
      ...
      it('can decode a log with an IDL file', async () => {
        ...
        const tx = blockData.transactions.find(tx => tx.transaction.signatures.find(s => s === '5Z18NZWUiDmxmVYncvuyACB9HRRYyzZfRPE9pfT2yaTpAveDTUwghWaYMPRk9Df5HsJy9yd6dBrndrmHz1zfsAig'))

        const programLogs = tx?.meta?.logMessages?.filter(l => l.startsWith('Program data:'))
        expect(programLogs?.length).toBe(1);

        const decoded = await decodeLog(solanaApi, programLogs![0], IDL_swap);
      ...
      it('can decode a log with an IDL found on chain', async () => {
        ...
        const decoded = await decodeLog(solanaApi, programLogs![0], undefined, 'swapFpHZwjELNnjvThjajtiVmkz3yPQEHjLtka2fwHW');
